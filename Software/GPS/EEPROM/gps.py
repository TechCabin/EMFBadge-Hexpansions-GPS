...
class GPSApp(app.App):
    ...
    VERSION = 4 # Increment this when making changes to the app that require the hexpansion app to be re-flashed with the new code.
            ...
            try:
                ...
                if p[0] == "$GPRMC" or p[0] == "$GNRMC":
                    if p[2] == "A":
                        ...
                        self.z = 0
                        # Set the RTC using the GPS time if we've not already done that once.
                        if not self.timeSet:
                            RTC().datetime((2000 + int(p[9][4:6]), int(p[9][2:4]), int(p[9][0:2]), 0, int(p[1][0:2]), int(p[1][2:4]), int(p[1][4:6]), 0))
                            self.timeSet = True
                    else:
                        self.red.toggle()
